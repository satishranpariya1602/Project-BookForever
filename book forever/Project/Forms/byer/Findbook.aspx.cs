...
public partial class Forms_Visitor_Default : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("~/Forms/byer/searchedbooksub.aspx?book=" + Session["book"] );
        }
        if (RadioButtonList1.SelectedItem.Value.ToString() == "categoery")
        {
            Response.Redirect("~/Forms/byer/searchedbookcat.aspx?book=" + Session["book"]);
        }
        if (RadioButtonList1.SelectedItem.Value.ToString() == "author")
        {
            Response.Redirect("~/Forms/byer/searchedbookaut.aspx?book=" + Session["book"]);
        }
    }
}
